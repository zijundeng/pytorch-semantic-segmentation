...
def main(train_args):
    net = train_args['model'](num_classes=cityscapes.num_classes).cuda()
    ...
    criterion = CrossEntropyLoss2d(ignore_index=cityscapes.ignore_label).cuda()
...
def validate(val_loader, net, criterion, optimizer, epoch, train_args, restore, visualize):
    ...
    for vi, data in enumerate(val_loader):
        ...
        outputs = net(inputs)
        predictions = outputs.data[:, :cityscapes.num_classes - 1, :, :].max(1)[1].squeeze_(1).cpu().numpy()
